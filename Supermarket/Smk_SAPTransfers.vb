...
Public Class Smk_SAPTransfers
    ...
    Public Function UpdateStock() As Boolean
        LoadingScreen.Show()
        Try
            SQL.Current.Execute("UPDATE Smk_SAPTransfers SET Posted = 1, PostedDate = GetDate(), AdjustmentQuantity = 0, DocumentNumber = 'Delta Validation' WHERE Posted = 0 AND FromSloc = ToSloc")
            SQL.Current.Execute("UPDATE Smk_SAPTransfers SET Posted = 1, PostedDate = GetDate(), AdjustmentQuantity = 0, DocumentNumber = 'Delta Validation' WHERE Posted = 0 AND ROUND(Quantity,3) <= 0")
            ...
            If Not SQL.Current.Execute(String.Format("INSERT INTO Smk_tmpSAPTransfers (t_Partnumber,t_FromSloc,t_ToSloc,t_Quantity,t_NewQuantity,t_UoM,t_Serialnumber,t_TransferID,t_FlagTransfer,t_Hostname,t_Date) SELECT S.Partnumber,T.FromSloc,T.ToSloc,T.Quantity,CASE WHEN S.UoM = 'FT' THEN T.Quantity*0.3048 ELSE CASE WHEN S.UoM = 'LB' THEN T.Quantity*0.45359237 ELSE T.Quantity END END,S.UoM,S.Serialnumber,T.ID,1,'{0}',GETDATE() " & _
                      "FROM Smk_SAPTransfers AS T INNER JOIN Smk_SerialMovements AS M ON T.MovementID = M.ID INNER JOIN Smk_Serials AS S ON M.SerialID = S.ID LEFT OUTER JOIN Smk_tmpSAPTransfers ON T.ID = t_TransferID WHERE t_TransferID IS NULL AND Posted = 0 AND FromSloc = '{1}' AND FromSloc <> ToSloc", Environment.MachineName, selected_sloc)) Then
                If SQL.Current.GetScalar(String.Format("SELECT COUNT(*) FROM Smk_SAPTransfers WHERE Posted = 0 AND FromSloc = '{0}' AND FromSloc <> ToSloc", selected_sloc)) > 0 Then
                    LoadingScreen.Hide()
    ...
    Public Function Transfer() As Boolean
        ...
        Try
            ...
            If sap.Available Then
                For j = 0 To Slocs.Count - 1
                    For Each row As DataRow In SQL.Current.GetDatatable(String.Format("SELECT t_Partnumber,sum_transfer,base_uom FROM Smk_tmpZapi INNER JOIN (SELECT t_Partnumber,SUM(t_NewQuantity) AS sum_transfer,CASE WHEN t_UoM = 'FT' THEN 'M' ELSE CASE WHEN t_UoM = 'LB' THEN 'KG' ELSE t_UoM END END AS base_uom FROM Smk_tmpSAPTransfers WHERE t_FlagTransfer = 1 AND t_ToSloc = '{0}' AND t_Hostname = '{1}' AND t_FromSloc = '{2}' GROUP BY t_Partnumber,CASE WHEN t_UoM = 'FT' THEN 'M' ELSE CASE WHEN t_UoM = 'LB' THEN 'KG' ELSE t_UoM END END HAVING SUM(t_NewQuantity) > 0) AS Transfers ON z_partNumber = t_Partnumber WHERE z_Hostname = '{1}' AND z_Quantity - sum_transfer >= 0", Slocs(j), Environment.MachineName, selected_sloc)).Rows
                        ...
                        UoMs.Add(row.Item("base_uom"))
                    Next
                    Dim doc_number As String = "--"
                    If Partnumbers.Count > 0 Then
                        doc_number = sap.MB1B(Parameter("SYS_PlantCode"), selected_sloc, Slocs(j), Partnumbers, Quantities, UoMs, If(selected_sloc = "0002", "311", "Z11"))
                        If doc_number.ToUpper.Contains("DOCUMENT") And doc_number.ToUpper.Contains("POSTED") Then
                            If Not SQL.Current.Execute(String.Format("UPDATE Smk_SAPTransfers SET Posted = 1, AdjustmentQuantity=Quantity-CASE WHEN t_UoM ='FT' THEN t_NewQuantity/0.3048 ELSE CASE WHEN t_UoM = 'LB' THEN t_NewQuantity/0.45359237 ELSE t_NewQuantity END END, PostedDate = GetDate(), DocumentNumber = '{0}', PostedUsername = '{4}' FROM Smk_tmpSAPTransfers WHERE ID = t_TransferID AND t_FlagTransfer = 1 AND ToSloc = '{1}' AND t_Hostname = '{2}' AND FromSloc = '{3}' AND t_Partnumber IN (" & _
                                ...
                                '************************ OTRA VEZ POR SI HAY PROBLEMA DE CONEXION ***********************************************
                                If Not SQL.Current.Execute(String.Format("UPDATE Smk_SAPTransfers SET Posted = 1, AdjustmentQuantity=Quantity-CASE WHEN t_UoM ='FT' THEN t_NewQuantity/0.3048 ELSE CASE WHEN t_UoM = 'LB' THEN t_NewQuantity/0.45359237 ELSE t_NewQuantity END END, PostedDate = GetDate(), DocumentNumber = '{0}', PostedUsername = '{4}' FROM Smk_tmpSAPTransfers WHERE ID = t_TransferID AND t_FlagTransfer = 1 AND ToSloc = '{1}' AND t_Hostname = '{2}' AND FromSloc = '{3}' AND t_Partnumber IN (" & _
                                                                     "SELECT t_Partnumber FROM Smk_tmpZapi INNER JOIN (SELECT t_Partnumber,SUM(t_NewQuantity) AS sum_transfer FROM Smk_tmpSAPTransfers WHERE t_FlagTransfer = 1 AND t_ToSloc = '{1}' AND t_Hostname = '{2}' AND t_FromSloc = '{3}' GROUP BY t_Partnumber HAVING SUM(t_NewQuantity) > 0) AS Transfers ON z_partNumber = t_Partnumber AND z_Hostname = '{2}' WHERE z_quantity-sum_transfer >= 0)", doc_number.ToUpper.Replace("DOCUMENT", "").Replace("POSTED", "").Trim, Slocs(j), Environment.MachineName, selected_sloc, User.Current.Username)) Then
                                    LoadingScreen.Hide()
        ...
        Catch ex As Exception
            ...
            SQL.Current.Execute(String.Format("DELETE FROM Smk_tmpZapi WHERE z_Hostname = '{0}';", Environment.MachineName))
            LoadingScreen.Hide()
            MsgBox("Hubo un problema al tratar de transferir la informacion a SAP." & vbCrLf & ex.Message, MsgBoxStyle.Critical, APP)
            Return False
        End Try
    End Function
    ...
    Private Sub CancelContextMenuStrimItem_Click(sender As Object, e As EventArgs) Handles CancelContextMenuStrimItem.Click
        If MessageBox.Show("¿Cancelar definitivamente la transferencia a SAP?" & vbNewLine & "Esta transferencia no volvera a aparecer.", "Eliminar", MessageBoxButtons.YesNoCancel, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
            SQL.Current.Delete("Smk_tmpSAPTransfers", "t_TransferID", id_cancel)
            SQL.Current.Execute(String.Format("UPDATE Smk_SAPTransfers SET Posted = 1, DocumentNumber = 'Canceled by {0}', PostedDate = GETDATE() WHERE ID = {1};", User.Current.Username, id_cancel))
