Public Class Rec_TrackerDiscountHistory
    ...
    Private Sub Find_btn_Click(sender As Object, e As EventArgs) Handles Find_btn.Click
        ...
        If Partnumber_rb.Checked Then
            If Partnumber_txt.Text.Trim.Length = 8 Then
                Movements_dgv.DataSource = SQL.Current.GetDatatable(String.Format("SELECT S.SerialNumber AS Serie,S.Partnumber AS [No. de Parte], D.[Description] AS Movimiento,M.[Location] AS Localizacion,M.Quantity AS Descuento,S.UoM AS Unidad,O.Fullname AS Operador,M.[Date] AS Fecha,T.Posted AS [Trasferido],T.Quantity+AdjustmentQuantity AS [Cantidad Transferida],T.FromSloc AS [De Sloc],T.ToSloc AS [Al Sloc],DocumentNumber AS [Numero de Documento],U.Fullname AS [Transferido por] FROM [Smk_SerialMovements] AS M INNER JOIN Smk_MovementsDescription AS D ON M.Movement = D.Movement INNER JOIN Smk_Serials AS S ON M.SerialID = S.ID LEFT OUTER JOIN Smk_SAPTransfers AS T ON M.ID = T.MovementID LEFT OUTER JOIN Smk_Operators AS O ON M.Badge = O.Badge LEFT OUTER JOIN Sys_Users AS U ON T.PostedUsername = U.Username WHERE S.Partnumber = '{0}' AND CONVERT(DATE,M.[Date]) BETWEEN '{1}' AND '{2}' ORDER BY M.[Date] DESC;", Partnumber_txt.Text, From_dtp.Value.ToString("yyyy-MM-dd"), To_dtp.Value.ToString("yyyy-MM-dd")), "Movimientos")
            ElseIf Partnumber_txt.Text.Trim <> "" Then
                FlashAlerts.ShowInformation("Captura el numero de parte completo.")
            Else
                Movements_dgv.DataSource = SQL.Current.GetDatatable(String.Format("SELECT S.SerialNumber AS Serie,S.Partnumber AS [No. de Parte], D.[Description] AS Movimiento,M.[Location] AS Localizacion,M.Quantity AS Descuento,S.UoM AS Unidad,O.Fullname AS Operador,M.[Date] AS Fecha,T.Posted AS [Trasferido],T.Quantity+AdjustmentQuantity AS [Cantidad Transferida],T.FromSloc AS [De Sloc],T.ToSloc AS [Al Sloc],DocumentNumber AS [Numero de Documento],U.Fullname AS [Transferido por] FROM [Smk_SerialMovements] AS M INNER JOIN Smk_MovementsDescription AS D ON M.Movement = D.Movement INNER JOIN Smk_Serials AS S ON M.SerialID = S.ID LEFT OUTER JOIN Smk_SAPTransfers AS T ON M.ID = T.MovementID LEFT OUTER JOIN Smk_Operators AS O ON M.Badge = O.Badge LEFT OUTER JOIN Sys_Users AS U ON T.PostedUsername = U.Username WHERE CONVERT(DATE,M.[Date]) BETWEEN '{0}' AND '{1}' ORDER BY M.[Date] DESC;", From_dtp.Value.ToString("yyyy-MM-dd"), To_dtp.Value.ToString("yyyy-MM-dd")), "Movimientos")
            End If
        ElseIf Serial_rb.Checked Then
            If SMK.IsSerial(Serial_txt.Text) Then
                Movements_dgv.DataSource = SQL.Current.GetDatatable(String.Format("SELECT S.SerialNumber AS Serie,S.Partnumber AS [No. de Parte], D.[Description] AS Movimiento,M.[Location] AS Localizacion,M.Quantity AS Descuento,S.UoM AS Unidad,O.Fullname AS Operador,M.[Date] AS Fecha,T.Posted AS [Trasferido],T.Quantity+AdjustmentQuantity AS [Cantidad Transferida],T.FromSloc AS [De Sloc],T.ToSloc AS [Al Sloc],DocumentNumber AS [Numero de Documento],U.Fullname AS [Transferido por] FROM [Smk_SerialMovements] AS M INNER JOIN Smk_MovementsDescription AS D ON M.Movement = D.Movement INNER JOIN Smk_Serials AS S ON M.SerialID = S.ID LEFT OUTER JOIN Smk_SAPTransfers AS T ON M.ID = T.MovementID LEFT OUTER JOIN Smk_Operators AS O ON M.Badge = O.Badge LEFT OUTER JOIN Sys_Users AS U ON T.PostedUsername = U.Username WHERE S.Serialnumber = '{0}' AND CONVERT(DATE,M.[Date]) BETWEEN '{1}' AND '{2}' ORDER BY M.[Date] DESC;", Serial_txt.Text, From_dtp.Value.ToString("yyyy-MM-dd"), To_dtp.Value.ToString("yyyy-MM-dd")), "Movimientos")
    ...
    Private Sub Rec_TrackerDiscountHistory_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        Serial_txt.Clear()
        Serial_txt.Focus()
        From_dtp.Value = CurrentDate.AddMonths(-12)
    End Sub

    Private Sub Rec_TrackerDiscountHistory_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Me.Dispose()
    End Sub
