Public Class MAn_ConsumptionHistory
    ...
    '    Dim movements As DataTable = SQL.Current.GetDatatable(String.Format("SELECT Partnumber AS [No. de Parte],CONVERT(DATE,[Date]) AS [Date],CASE WHEN Quantity > 0 THEN '1.Entrada' ELSE '2.Salida' END AS Clase, SUM(Quantity) AS Consumption " & _
    '                                                                          "FROM Smk_MB51 WHERE [Date] >= '{0}' AND Quantity > 0 AND Sloc = '{2}' GROUP BY Partnumber,CONVERT(DATE,[Date]),CASE WHEN Quantity > 0 THEN '1.Entrada' ELSE '2.Salida' END;", From_dtp.Value.ToString("yyyy-MM-dd"), partnumber_filter, Sloc_cbo.SelectedItem))
    ...
    '    Dim dates As DataTable = movements.DefaultView.ToTable(True, "Date")
    '    For Each d As DataRow In dates.Rows
    ...
    '    Dim pivoter_movements As New PivotTable(movements)
    '    Dim pivot_movement = pivoter_movements.PivotDates("No. de Parte", "Clase", "Consumption", AggregateFunction.Sum, "Date", "System.Decimal")
