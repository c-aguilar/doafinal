Public Class QLY_RemoveSerialAlert
    ...
    Private Sub RefreshAlerts()
        alerts = SQL.Current.GetDatatable("SELECT S.Serialnumber,S.Partnumber,R.SupplierPartnumber,R.SupplierName,dbo.Qly_AlertReasons(S.Partnumber, S.[Date]) AS Reason,S.[Date],S.Masternumber,~RedTag AS [Check] FROM vw_Smk_Serials AS S INNER JOIN Sys_RawMaterial AS R ON S.Partnumber = R.Partnumber WHERE S.RedTag = 1 AND S.[Status] <> 'D';")
