Public Class FGR_Operators

    Private Sub FGR_Operators_Load(sender As Object, e As EventArgs) Handles MyBase.Load
        GetOperators()
        GF.FillCombobox(Shift_cbo, SQL.Current.GetDatatable("SELECT Shift FROM Sys_Shifts ORDER BY Shift"), "Shift", "Shift")
        GF.FillCombobox(Role_cbo, SQL.Current.GetDatatable("SELECT ID, Role FROM Smk_OperatorRoles WHERE Area = 'FGR' ORDER BY Role"), "Role", "ID")
        For i = 1 To CInt(Parameter("SYS_BadgeLength", 9))
            Badge_txt.Mask &= "0"
        Next
    End Sub
    Private Sub GetOperators()
        GF.FillCombobox(Badge_cbo, SQL.Current.GetDatatable("SELECT '#########' AS Badge,'(Nuevo)' AS FullName UNION ALL SELECT Badge, FullName FROM Smk_Operators WHERE Area = 'FGR' AND Active = 1 ORDER BY FullName"), "FullName", "Badge")
    End Sub
    ...
    Private Sub Save_btn_Click(sender As Object, e As EventArgs) Handles Save_btn.Click
                ...
                If Shift_cbo.SelectedIndex > -1 Then
                    If Firstname_txt.Text <> "" AndAlso Lastname_txt.Text <> "" Then
                        If SQL.Current.Exists("Smk_Operators", "Badge", Badge_txt.Text) Then
                            If SQL.Current.Exists("Smk_Operators", {"Badge", "Area"}, {Badge_txt.Text, "FGR"}) Then
                            ...
                            ElseIf SQL.Current.Exists("Smk_Operators", {"Badge", "Active"}, {Badge_txt.Text, 0}) Then
                                SQL.Current.Update("Smk_Operators", {"Firstname", "Lastname", "Shift", "Active", "Area", "RoleID", "Activity", "Department"}, {Firstname_txt.Text, Lastname_txt.Text, Shift_cbo.SelectedValue, 1, "FGR", NullReplace(Role_cbo.SelectedValue, DBNull.Value), Activity_txt.Text, Department_txt.Text}, "Badge", Badge_txt.Text)
                                Delta.SaveUserPhoto(Badge_txt.Text, Picture_pb.Image)
                                GetOperators()
                                Clean()
                                FlashAlerts.ShowConfirm("Registrado.")
                            ...
                            End If
                        Else
                            If SQL.Current.Insert("Smk_Operators", {"Badge", "Firstname", "Lastname", "Area", "Shift", "RoleID", "Activity", "Department"}, {Badge_txt.Text, Firstname_txt.Text, Lastname_txt.Text, "FGR", Shift_cbo.SelectedValue, NullReplace(Role_cbo.SelectedValue, DBNull.Value), Activity_txt.Text, Department_txt.Text}) Then
                                Delta.SaveUserPhoto(Badge_txt.Text, Picture_pb.Image)
                                GetOperators()
                                Clean()
                                FlashAlerts.ShowConfirm("Registrado.")
        ...
        Else
            If Shift_cbo.SelectedIndex > -1 Then
                If Firstname_txt.Text <> "" AndAlso Lastname_txt.Text <> "" Then
                    SQL.Current.Update("Smk_Operators", {"Firstname", "Lastname", "Shift", "Active", "Area", "RoleID", "Activity", "Department"}, {Firstname_txt.Text, Lastname_txt.Text, Shift_cbo.SelectedValue, 1, "FGR", NullReplace(Role_cbo.SelectedValue, DBNull.Value), Activity_txt.Text, Department_txt.Text}, "Badge", Badge_cbo.SelectedValue)
    ...
    Private Sub Delete_btn_Click(sender As Object, e As EventArgs) Handles Delete_btn.Click
        If Badge_cbo.SelectedValue <> "#########" Then
            If MessageBox.Show("¿Seguro de eliminar este operador?", "Confirmar", MessageBoxButtons.YesNo, MessageBoxIcon.Question) = Windows.Forms.DialogResult.Yes Then
    ...
    Private Sub FGR_Operators_FormClosed(sender As Object, e As FormClosedEventArgs) Handles MyBase.FormClosed
        Me.Dispose()
    End Sub
End Class