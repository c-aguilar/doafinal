        ...
        Select Case option_scan
            ...
            Case Else
                If CDR.Carts.Exists(Function(f) f.ID.ToLower = option_scan.ToLower) Then
                    ...
                    cart.Refresh()
                    If cart.Status = CDR.Status.Delivering OrElse cart.Status = CDR.Status.Out Then
                    ...
                    Else
                        ...
                        Cart_txt.Clear()
                        Cart_txt.Focus()
                        FlashAlerts.ShowError("El carro no se encuentra afuera.", 1, True)
                    End If
                Else
                End If
        End Select
    ...
    Private Sub ReadBadge()
        ...
        Else
            ...
            If SQL.Current.Exists(String.Format("SELECT Badge FROM Smk_Operators WHERE Badge = '{0}' AND Active = 1 AND Area IN ('SMK','REC','CDR')", Strings.Right(Badge_txt.Text, Parameter("SYS_BadgeLength", 9)))) Then
                ...
                If cart.Status = CDR.Status.Delivering Then
                    'CERRAR LA ULTIMA VUELTA
                    SQL.Current.Execute(String.Format("UPDATE DDR_CartsLoopRegister SET DeliveryEndDate = GETDATE() WHERE ID = {0}", cart.CurrentLoopID))
                End If

                'REGISTRAR LLEGADA VACIO
                SQL.Current.Insert("DDR_CartsLoopRegister", {"Cart", "CollectingBadge"}, {CartID, badge})
                cart.Refresh()
